...
class TestStringPairFeatureExtractor(unittest.TestCase):
    def test_transform_binary(self):
        ...
        # k . . . .
        #   c a t 2
        expected_x = np.zeros((4, 4, 3))
        expected_x[:, 0, 0] = 1.0
        expected_x[0, :, 0] = 1.0
        expected_x[1, 1, 1] = 1.0
        expected_x[2, 2, 1] = 1.0
        expected_x[3, 3, 2] = 1.0

        test_extractor = StringPairFeatureExtractor(start=True, match=True, numeric=True)
    ...
    def test_transform_transition(self):
        ...
        nchars = len(chars)
        print nchars
        expected_x = np.zeros((2, 2, len(chars)**2))
        expected_x[0, 0, 2 + nchars * 1] = 1.0  # b->c
        expected_x[0, 1, 0 + nchars * 1] = 1.0  # b->a
        expected_x[1, 0, 2 + nchars * 0] = 1.0  # a->c
        expected_x[1, 1, 0 + nchars * 0] = 1.0  # a->a
